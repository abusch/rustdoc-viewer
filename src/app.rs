...
use crate::page::{self, ImplGroup, Page};
...
impl App {
    ...
    pub fn focus_next_target(&mut self) {
        ...
        let next = match self.focus {
            Some(f) if f + 1 < p.targets.len() => Some(f + 1),
            Some(_) => None,
            None => p.targets.iter().position(|t| t.line >= start && t.line < end),
        };
        self.focus = next.or_else(|| p.targets.iter().position(|t| t.line >= start));
